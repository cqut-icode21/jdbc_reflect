...
public class SqlDaoImpl implements SqlDao {
     ...
     *
     * @param clazz 类
     * @return 查询所有的SQL语句的字符串:select sno(tno),sname(tname),sage(tage) from student(teacher)
     */
    @Override
     ...
     *
     * @param clazz 类
     * @return 根据id查询的SQL语句字符串:select sno(tno),sname(tname),sage(tage) form student(teacher) where sno(tno) = ?
     */
    @Override
    ...
    public String addSql(Class<?> clazz) {
        ...
        String table = tableAnnotation.tableName();
        sql.append(table).append(" (");

        for (Field field : fields){
            Column columnAnnotation = field.getAnnotation(Column.class);
            String column = columnAnnotation.columnName();
